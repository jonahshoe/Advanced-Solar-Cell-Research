...
meg_cap = 2 #maximum number of electrons that can be generated by a high-energy photon for multiexciton and multijunctionMEG
multijunctionMEG = 1 #multijunctionMEG is still a work in progress
...
if multijunction == 1:
    ...
    if Joules == 1:
        ...
        eg1_2 = np.linspace(eglimit3,eglimit4,egcount)*q
    ...
    if am1pt5 == 1:
        ...
        maxEfficiencyMJReal = []
        j1Real2 = np.zeros([egcount,vcount,vcount])
        j2Real2 = np.zeros([egcount,vcount,vcount])
    ...
    for i in range(egcount):
        if Joules == 1:
            ...
            v1_2 = np.linspace(0,eg1_2-0.1,vcount)
            v2 = np.linspace(0,eg2-0.1,vcount)
            v2_2 = np.linspace(0,eg2_2[i]-0.1,vcount)
        if eV == 1:
            v1 = np.linspace(0,eg1[i]-0.1,vcount)/q
            v1_2 = np.linspace(0,eg1_2-0.1,vcount)/q
            v2 = np.linspace(0,eg2-0.1,vcount)/q
            v2_2 = np.linspace(0,eg2_2[i]-0.1,vcount)/q
        ...
        if am1pt5 == 1:
            ...
            term1_2_2Real = realsolar(wavelength,radiance,eg2_2[i],eg1_2)
            term1_1_2Real = realsolar(wavelength,radiance,eg1_2,np.inf)

        term2_2 = (1-f*C)*Q(eg2,eg1[i],Tc)
        term2_1 = (1-f*C)*Q(eg1[i],np.inf,Tc)
        term2_2_2 = (1-f*C)*Q(eg2[i],eg1,Tc)
        term2_1_2 = (1-f*C)*Q(eg1,np.inf,Tc)
        ...
        while(len(index1Real) == 0):
            print("trying again")
            for j in range(vcount):
                term3_1_2 = Q_V(eg1_2,Tc,v1_2[j])
                term4_2_2 = Q_V(eg1_2,Tc,v1_2[j])

                for n in range(vcount):
                    term4_1_2 = Q_V(eg1_2,Tc,v2_2[n])
                    term3_2_2 = Q_V(eg2_2[i],Tc,v2_2[n])

                    if am1pt5 == 1:
                        j1_2Real[i,j,n] = q*term1_1_2Real+qg*(term2_1_2-term3_1_2+term4_1_2)
                        if j1_2Real[i,j,n] < 0:
                            j1_2Real[i,j,n] = 0
                        j2_2Real[i,j,n] = q*term1_2_2Real+qg*(term2_2_2-term3_2_2+term4_2_2)
                        if j2_2Real[i,j,n] < 0:
                            j2_2Real[i,j,n] = 0
                        diffReal2[j,n] = np.abs(j1_2Real[i,j,n] - j2_2Real[i,j,n])
                        javgReal2 = (j1_2Real[i,j,n]+j2_2Real[i,j,n])/2
                        if javgReal2 != 0 and np.isnan(diffReal2[j,n]) == False:
                            if diffReal2[j,n]/javgReal2 < threshold:
                                index1_2Real.append(j)
                                index2_2Real.append(n)
            threshold2 += step2
        ...
        if am1pt5 == 1:
            print(str(len(index1Real)) + " pairs found for " + str(eg1[i]) + " eV with threshold = " + str(threshold-step))
            print(str(len(index1_2Real)) + " pairs found for " + str(eg1_2) + " eV with threshold = " + str(threshold2-step2))
            if len(index1Real) > 0:
            ...
            if len(index1_2Real) > 0:
                for b in range(len(index1_2Real)):
                    j1_2tempReal = j1_2Real[i,index1_2Real[b],index2_2Real[b]]
                    j2_2tempReal = j2_2Real[i,index1_2Real[b],index2_2Real[b]]
                ...
                maxEfficiencyMJReal2.append(np.nan)
    plt.figure(fignum)
    fignum += 1
    if blackbody == 1:
...
if multijunctionMEG == 1:
    ...
    for i in range(egcount):
        ...
        max_meg_layer2 = int(eg1[i]/eg2) #layer 2 will only undergo MEG if it has more than one multiple of its bandgap between its bandgap and layer 1's bandgap
        ...
        if am1pt5 == 1:
            ...
            index2Real = []
            efficiencyReal = []
            term1_1Real = np.zeros([meg_cap])
            term1_2Real = np.zeros([max_meg_layer2])
            diffReal = np.zeros_like(v1)
            for u in range(meg_cap):
                if u+1 < meg_cap:
                    term1_1Real[u] = realsolar(wavelength,radiance,(u+1)*eg1[i],(u+2)*eg1[i])
                elif u+1 == meg_cap:
                    term1_1Real[u] = realsolar(wavelength,radiance,(u+1)*eg1[i],np.inf)
            for u in range(max_meg_layer2):
                if u+1 < max_meg_layer2:
                    term1_2Real[u] = (u+1)*realsolar(wavelength,radiance,(u+1)*eg2,(u+2)*eg2)
                elif u+1 == max_meg_layer2:
                    term1_2Real[u] = (u+1)*realsolar(wavelength,radiance,(u+1)*eg2,eg1[i])
            storedDiffsReal = []

        term2_1 = np.zeros([meg_cap])
        term2_2 = np.zeros([max_meg_layer2])

        for u in range(meg_cap):
            if u+1 < meg_cap:
                term2_1[u] = (u+1)*(1-f*C)*Q(eg1[i],2*eg1[i],Tc)
            elif u+1 == meg_cap:
                term2_1[u] = (u+1)*(1-f*C)*Q(eg1[i],np.inf,Tc)

        for u in range(max_meg_layer2):
            if u+1 < max_meg_layer2:
                term2_2[u] = (u+1)*(1-f*C)*Q((u+1)*eg2,(u+2)*eg2,Tc)
            elif u+1 == max_meg_layer2:
                term2_2[u] = (u+1)*(1-f*C)*Q((u+1)*eg2,eg1[i],Tc)
        ...
        while(len(index1Real) == 0):
            print("trying again")
            for j in range(vcount):
                term3_1 = np.zeros([meg_cap])
                term4_2 = np.zeros([meg_cap])

                for u in range(meg_cap):
                    if u+1 < meg_cap:
                        term3_1[u] = (u+1)*Q_V2((u+1)*eg1[i],(u+2)*eg1[i],Tc,v1[j])
                    elif u+1 == meg_cap:
                        term3_1[u] = (u+1)*Q_V2((u+1)*eg1[i],np.inf,Tc,v1[j])

                for u in range(max_meg_layer2):
                    if u+1 < meg_cap: #use meg_cap since this one goes to infinity
                        term4_2[u] = (u+1)*Q_V2(eg1[i]+u*eg2,eg1[i]+(u+1)*eg2,Tc,v1[j]) ### even though the integral's lower limit is Eg1, it still goes by multiples of eg2 for layer 2's infinite integrals
                    elif u+1 == meg_cap:
                        term4_2[u] = (u+1)*Q_V2(eg1[i]+u*eg2,np.inf,Tc,v1[j])

                for n in range(vcount):
                    j1Real[i,j,n] = 0 #reset for each iteration of the while loop
                    j2Real[i,j,n] = 0
                    term4_1 = np.zeros([meg_cap])
                    term3_2 = np.zeros([meg_cap])

                    for u in range(meg_cap):
                        if u+1 < meg_cap:
                            term4_1[u] = (u+1)*Q_V2((u+1)*eg1[i],(u+2)*eg1[i],Tc,v2[n])
                        elif u+1 == meg_cap:
                            term4_1[u] = (u+1)*Q_V2((u+1)*eg1[i],np.inf,Tc,v2[n])

                    for u in range(meg_cap):
                        if u+1 < meg_cap:
                            term3_2[u] = (u+1)*Q_V2((u+1)*eg2,(u+2)*eg2,Tc,v2[n])
                        elif u+1 == meg_cap:
                            term3_2[u] = (u+1)*Q_V2((u+1)*eg2,np.inf,Tc,v2[n])
                    ...
                    if am1pt5 == 1:
                        for u in range(meg_cap):
                            j1Real[i,j,n] += q*term1_1Real[u]+qg*(term2_1[u]-term3_1[u]+term4_1[u])
                            j2Real[i,j,n] += qg*(term4_2[u]-term3_2[u])
                        if j1Real[i,j,n] < 0:
                            j1Real[i,j,n] = 0
                        for u in range(max_meg_layer2):
                            j2Real[i,j,n] += q*term1_2Real[u]+qg*term2_2[u]
                        if j2Real[i,j,n] < 0:
                            j2Real[i,j,n] = 0
            ...
            threshold += step #gradually loosen the series constraint until it coughs up a current density pair
