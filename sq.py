...
meg_cap = 2 #maximum number of electrons that can be generated by a high-energy photon for multiexciton
            ...
            for j in range(vcount):
                ...
                for u in range(max_meg_layer2):
                    if u+1 < meg_cap: #use meg_cap since this one goes to infinity
                        term4_2[i] = (u+1)*Q_V2(eg1[i]+u*eg2,eg1[i]+(u+1)*eg2,Tc,v1[j]) ### even though the integral's lower limit is Eg1, it still goes by multiples of eg2 for layer 2's infinite integrals
                    elif u+1 == meg_cap:
                        term4_2[i] = (u+1)*Q_V2(eg1[i]+u*eg2,np.inf,Tc,v1[j])
